...
const createFeeStructure = async (req, res) => {
  try {
    const { grade: rawGrade, amount, description, branchId } = req.body;
    if (!rawGrade || !amount) {
    ...
    // Check if a fee for this grade already exists for the CURRENT academic year and branch
...
const generateMonthlyFees = async (req, res) => {
  try {
    const { month, dueDate, description, branchId } = req.body;
    if (!month) {
    ...
    // Only check existing fees for students in the filtered set
