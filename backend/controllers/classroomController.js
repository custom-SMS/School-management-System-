...
const getAttendanceSessions = async (req, res) => {
  ...
  try {
    ...
    const sessions = await prisma.attendance.findMany({
      ...
      where: req.branchFilter ? { class: { ...req.branchFilter } } : {},
  ...
  try {
    ...
    for (let data of gradesData) {
      ...
      if (compWeights.length > 0) {
        ...
        // Normalized total on 0-100 scale

        total = Number((weightedSum / sumWeights).toFixed(2));
      ...
      // Only include non-null assessment fields

      if (quiz != null) updateData.quiz = quiz;

      if (assignment != null) updateData.assignment = assignment;

      if (test != null) updateData.test = test;

      if (midterm != null) updateData.midterm = midterm;

      if (final != null) updateData.final = final;
