  ...
  return (
    <CashierLayout searchPlaceholder="Search student records...">
      <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div>
          <h1 className="text-2xl sm:text-3xl font-black tracking-tight text-slate-900">Debt Oversight</h1>
        ...
        <div className="flex flex-col gap-2 sm:flex-row sm:gap-3">
          <button
            ...
            className="flex items-center justify-center gap-2 rounded-xl border border-slate-200 px-4 py-2.5 text-sm font-bold text-slate-600"
          >
          ...
          <button
            ...
            className="flex items-center justify-center gap-2 rounded-xl bg-slate-900 px-4 py-2.5 text-sm font-bold text-white disabled:opacity-50"
          >
      ...
      <div className="mt-6 grid grid-cols-1 gap-6 lg:grid-cols-1 xl:grid-cols-[1.6fr_1fr]">
        <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <h2 className="text-lg sm:text-xl font-bold text-slate-900">Collection by Grade</h2>
          ...
          {byGrade.length === 0 ? (
            ...
            <div className="mt-8 flex h-48 sm:h-56 items-stretch gap-2 sm:gap-6 overflow-x-auto pb-2">
              {byGrade.map((g) => {
                ...
                return (
                  <div key={g.className} className="flex min-w-[40px] flex-1 flex-col items-center gap-2">
                    <div className="flex w-full flex-1 items-end justify-center">
                      <div
                        className="flex w-full max-w-10 sm:max-w-12 flex-col-reverse overflow-hidden rounded-t-lg bg-rose-300"
                    ...
                    <span className="text-[10px] sm:text-xs font-medium text-slate-400">{g.className}</span>
                  </div>
                );
              })}
            </div>
      ...
      <section className="mt-6 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
        <div className="mb-5 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:gap-2 text-sm">
            ...
            <select value={targetMonth} onChange={(e) => setTargetMonth(e.target.value)} className="rounded-lg border border-slate-200 px-3 py-1.5 font-semibold text-slate-600 text-sm">
          ...
          <table className="w-full min-w-[500px] text-left text-sm">
            <thead>
              <tr className="border-b border-slate-200 text-xs uppercase tracking-wide text-slate-400">
                ...
                <th className="py-3 pr-4 font-semibold hidden sm:table-cell">Contact</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-100">
              {defaulters.map((d) => (
                <tr key={d._id || d.id} className="text-slate-700">
                  <td className="py-4 pr-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div className="min-w-0">
                        <div className="font-bold text-slate-900 truncate">{d.user?.name || '—'}</div>
                  ...
                  <td className="py-4 pr-4 text-slate-500 hidden sm:table-cell">{d.user?.email || '—'}</td>
                </tr>
              ))}
              {defaulters.length === 0 && (
