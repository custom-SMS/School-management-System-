    ...
    <CashierLayout searchPlaceholder="Search students, receipts...">
      <div className="mb-6 flex flex-col gap-1">
        ...
        <p className="text-sm text-slate-500">
          Real-time revenue tracking &mdash; <span className="font-semibold text-slate-700">{periodLabel}</span>
        </p>
      </div>
      ...
      {error ? (
        ...
        <>
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
            <StatCard label="Total Revenue" value={`ETB ${etb(totalRevenue)}`} sub={periodLabel} />
            <StatCard label="Outstanding Balance" value={`ETB ${etb(outstanding)}`} badgeTone="rose" sub={periodLabel} />
            ...
            <StatCard label="Collection Rate" value={`${collectionRate}%`} sub={`ETB ${etb(totalRevenue)} of ${etb(billed)} billed · ${periodLabel}`} />
          </div>
