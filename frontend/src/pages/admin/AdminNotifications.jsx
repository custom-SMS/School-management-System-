...
const AUDIENCES = [
  { value: 'all', label: 'All Branch Users' },
...
const TYPE_STYLES = {
  ...
  StudentMessage:'bg-cyan-50 text-cyan-700 border-cyan-100',
...
export default function AdminNotifications() {
  ...
  const fetchFeed = useCallback(async () => {
    ...
    try {
      ...
      toast.error(err.response?.data?.message || 'Failed to load notifications feed.');
    } finally {
      setLoading(false);
    }
  }, [page]);
  ...
  return (
    <AdminLayout pageTitle="Announcements & Notices">
      <div className="mb-6">
        <h2 className="text-2xl font-black tracking-tight text-slate-900">Announcements</h2>
        <p className="text-sm font-medium text-slate-500">Broadcast branch announcements to teachers, cashiers, parents, or students.</p>
      </div>
          ...
          <form onSubmit={handleBroadcast} className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm space-y-4">
            ...
            <div className="space-y-1.5">
              ...
              <input
                ...
                maxLength={120}
                placeholder="e.g. Branch Staff Meeting or School Notice"
                className="w-full rounded-lg border border-slate-200 bg-slate-50 px-3 py-2.5 text-sm outline-none focus:border-slate-400 focus:bg-white"
              />
            </div>
          ...
          <div className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
            ...
            <div className="divide-y divide-slate-100">
              {loading ? (
                ...
                <div className="px-6 py-12 text-center text-sm text-slate-400">No announcements sent yet.</div>
