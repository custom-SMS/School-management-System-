...
function FetchError({ onRetry, t }) {
  return (
    <div className="mt-6 rounded-2xl border border-rose-200 bg-rose-50 p-10 text-center">
      ...
      <p className="text-lg font-bold text-rose-700">{t('couldNotLoadAttendance')}</p>
...
export default function StudentAttendance() {
  ...
  return (
    <StudentLayout searchPlaceholder="Search records...">
      <h1 className="text-3xl font-black tracking-tight text-slate-900">{t('studentAttendanceRecord')}</h1>
      <p className="text-sm text-slate-500">{t('trackAttendance')}</p>
      ...
      {loading ? (
        ...
        <>
          <div className="mt-6 grid grid-cols-1 gap-6 lg:grid-cols-[1fr_1.6fr]">
            ...
            <div className="space-y-6">
              <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
                <div className="text-xs font-semibold uppercase tracking-wide text-slate-400">{t('overallAttendanceRateLabel')}</div>
                ...
                <div className="mt-5 grid grid-cols-3 gap-3 text-center">
                  <div><div className="text-xs font-semibold uppercase text-slate-400">{t('present')}</div><div className="text-lg font-black text-slate-900">{present}</div></div>
                  <div><div className="text-xs font-semibold uppercase text-slate-400">{t('absent')}</div><div className="text-lg font-black text-rose-600">{absent}</div></div>
                  <div><div className="text-xs font-semibold uppercase text-slate-400">{t('late')}</div><div className="text-lg font-black text-amber-600">{late}</div></div>
                </div>
              </div>

              {rate < 85 && (
                <div className="rounded-2xl border border-rose-200 bg-rose-50 p-5">
                  <div className="flex items-center gap-2 font-bold text-rose-700"><span>⚠</span> {t('lowAttendanceAlertStudent')}</div>
                  <p className="mt-1 text-sm text-rose-600">{t('lowAttendanceMsgStudent')}</p>
                </div>
              )}
              ...
              <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
                ...
                <div className="space-y-3 text-sm">
                  <div className="flex items-center justify-between"><span className="font-semibold text-slate-600">{t('totalSessions')}</span><span className="font-bold text-slate-900">{totalSessions}</span></div>
                  <div className="flex items-center justify-between"><span className="font-semibold text-slate-600">{t('recorded')}</span><span className="font-bold text-slate-900">{bd.recordedCount ?? present + absent + late}</span></div>
                </div>
              </div>
            </div>
          ...
          <section className="mt-6 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
            <h2 className="mb-4 text-lg font-bold text-slate-900">{t('recentAbsences')}</h2>
                  ...
                  {recent.map((a, i) => (
                    <tr key={i} className="text-slate-700">
                      ...
                      <td className="py-3 pr-4">
                        <span className={`rounded-md px-2.5 py-1 text-xs font-bold ${a.status === 'Absent' ? 'bg-rose-50 text-rose-700' : 'bg-amber-50 text-amber-700'}`}>
                          {statusLabel(a.status).toUpperCase()}
                        </span>
                      </td>
                    </tr>
                  ))}
                  {recent.length === 0 && <tr><td colSpan="2" className="py-8 text-center font-semibold text-emerald-600">{t('noPerfectRecord')}</td></tr>}
                </tbody>
              </table>
            </div>
          </section>
