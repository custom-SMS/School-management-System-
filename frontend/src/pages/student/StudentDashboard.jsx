...
function FetchError({ onRetry, t }) {
  return (
    <div className="mt-6 rounded-2xl border border-red-200 bg-[#fdf3f2] p-8 text-center text-[#c53929]">
      ...
      <p className="text-base font-bold text-[#c53929]">{t('couldNotLoadDashboard')}</p>
      <p className="mt-1 text-xs text-[#66889a]">{t('serverUnavailable')}</p>
      <button onClick={onRetry} className="mt-4 rounded-full bg-[#3b6b82] px-5 py-2 text-xs font-bold text-white transition hover:bg-[#203e4f]">{t('retry')}</button>
    </div>
  );
}
...
export default function StudentDashboard() {
  ...
  const name = stats?.profile?.user?.name || t('student');
  ...
  const statusLabel = (status) => {
    ...
    if (status === 'Permission') return t('permission');
    return status || t('absent');
  };
    ...
    <StudentLayout searchPlaceholder={t('searchPlaceholder')}>
      ...
      <div className="rounded-2xl bg-[#203e4f] p-6 sm:p-8 text-white shadow-md relative overflow-hidden">
        <div className="relative z-10">
          <h1 className="text-2xl sm:text-3xl font-extrabold tracking-tight">{t('helloStudent', { name: firstName })}</h1>
          <p className="mt-1 text-xs sm:text-sm text-[#cbe1eb]">{t('readyToStudy')}</p>
      ...
      {loading ? (
        ...
        <>
          <div className="mt-6 grid grid-cols-1 gap-6 xl:grid-cols-[1.6fr_1fr]">
            ...
            <section className="rounded-2xl border border-[#d8e6ed] bg-white p-6 shadow-sm">
              <div className="mb-4 flex items-center justify-between">
                <h2 className="text-base font-bold text-[#203e4f]">{t('todaysSchedule')}</h2>
                <Link to="/student/academics" className="text-xs font-bold text-[#3b6b82] hover:underline">{t('fullTimetable')}</Link>
              </div>
              <div className="space-y-3">
                {todaySlots.length ? todaySlots.slice(0, 4).map((s, i) => (
                  <div key={s.id || i} className={`rounded-xl border p-4 transition ${i === 0 ? 'border-[#3b6b82] bg-[#f0f6f9]' : 'border-[#e2edf2]'}`}>
                    <div className="text-[10px] font-bold uppercase tracking-wider text-[#63889b]">{i === 0 ? t('upNext') : `${t('period')} ${i + 1}`}</div>
                    <div className="mt-1 text-base font-bold text-[#203e4f]">{s.subject?.name || s.class?.name}</div>
                    <div className="text-xs text-[#547587] mt-0.5">{s.startTime} – {s.endTime}{s.room ? ` · ${t('room')} ${s.room}` : ''}</div>
                  </div>
                )) : (
                  <p className="py-8 text-center text-xs text-[#66889a]">{t('noClassesToday', { day: todayName })}</p>
                )}
              </div>
            </section>
            ...
            <section className="space-y-6">
              {gradingSettings.gpaEnabled ? (
                <div className="rounded-2xl border border-[#d8e6ed] bg-white p-6 text-center shadow-sm">
                  <div className="text-xs font-bold uppercase tracking-wider text-[#63889b]">{t('currentGpa')}</div>
                  {gpa != null ? (
                    <>
                      ...
                      <div className="mt-3 text-xs font-medium text-[#66889a]">{t('basedOnSubjects', { count: subjects.length, plural: subjects.length === 1 ? '' : 's' })}</div>
                    </>
                  ) : (
                    <>
                      ...
                      <div className="mt-3 text-xs font-medium text-[#66889a]">{t('pendingGrades')}</div>
                    </>
                  )}
                </div>
              ) : (
                <div className="rounded-2xl border border-[#d8e6ed] bg-white p-6 text-center shadow-sm">
                  <div className="text-xs font-bold uppercase tracking-wider text-[#63889b]">{t('averageScore')}</div>
                  {avgPct != null ? (
                    <>
                      ...
                      <div className={`mt-2 text-xs font-bold ${passStatus === t('pass') ? 'text-[#2d7a64]' : 'text-[#c53929]'}`}>{t('statusLabel', { status: passStatus })}</div>
                      <div className="mt-0.5 text-xs text-[#66889a]">{t('passMarkLabel', { mark: gradingSettings.passMark })}</div>
                    </>
                  ) : (
                    <>
                      <div className="mx-auto mt-3 flex h-24 w-24 items-center justify-center rounded-full border-4 border-[#e2edf2] text-sm font-bold text-[#88a7b8]">—</div>
              ...
              <div className="rounded-2xl border border-[#d8e6ed] bg-white p-6 text-center shadow-sm">
                <div className="text-xs font-bold uppercase tracking-wider text-[#63889b]">{t('attendance')}</div>
                <div className="mt-1 text-3xl font-black text-[#203e4f]">{attendanceRate}%</div>
                <div className="mt-1 text-xs font-medium text-[#66889a]">{attendanceRate >= 90 ? t('excellentConsistency') : t('keepItUp')}</div>
              </div>
            </section>
          </div>
          ...
          <section className="mt-6 rounded-2xl border border-[#d8e6ed] bg-white p-6 shadow-sm">
            <div className="mb-4 flex items-center justify-between">
              <h2 className="text-base font-bold text-[#203e4f]">{t('recentGradesLabel')}</h2>
              <Link to="/student/academics" className="text-xs font-bold text-[#3b6b82] hover:underline">{t('viewAll')}</Link>
            </div>
            <div className="divide-y divide-[#edf3f6]">
                ...
                return (
                  <div key={g._id || i} className="flex items-center justify-between gap-3 py-3">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <div className="text-xs font-bold text-[#203e4f]">{g.subject || t('subject')}</div>
              ...
              {grades.length === 0 && <p className="py-8 text-center text-xs text-[#66889a]">{t('noGradesYet')}</p>}
            </div>
          </section>
          ...
          <section className="mt-6 rounded-2xl border border-[#d8e6ed] bg-white p-6 shadow-sm">
            <div className="mb-4 flex items-center justify-between">
              <h2 className="text-base font-bold text-[#203e4f]">{t('attendanceHistory')}</h2>
              <Link to="/student/attendance" className="text-xs font-bold text-[#3b6b82] hover:underline">{t('viewFullHistory')}</Link>
            </div>
            <div className="overflow-x-auto">
              <table className="w-full text-left text-xs">
                <thead>
                  <tr className="border-b border-[#e2edf2] text-[10px] font-bold uppercase tracking-wider text-[#63889b]">
                    <th className="py-2.5 pr-4">{t('date')}</th>
                    <th className="py-2.5 pr-4">{t('status')}</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-[#f0f6f9]">
                  ...
                  {(stats?.attendance || []).length === 0 && (
                    <tr><td colSpan="2" className="py-8 text-center text-xs text-[#66889a]">{t('noAttendanceRecordsAvail')}</td></tr>
                  )}
                </tbody>
              </table>
            </div>
